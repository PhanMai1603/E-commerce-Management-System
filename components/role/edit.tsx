...
import { updateRole } from "@/app/api/role"; // Giả sử updateRole là hàm API để cập nhật vai trò
...
export default function RoleEditForm({ role, onSave, onCancel }: RoleEditFormProps) {
    ...
    const handleSubmit = async () => {
        if (!role || !role.name || !roleName.trim()) {
        ...
        try {
          // Chỉ gửi name nếu có thay đổi
          const updatedData: any = { permissions }; 
          if (roleName.trim() !== role.name.trim()) {
            updatedData.name = roleName.trim();
          }
    ...
    return (
        <div className="flex flex-col gap-4">
            <Card>
                ...
                <CardContent>
                    {/* Trường nhập liệu để chỉnh sửa tên vai trò */}
                    <Input
